...
public:
 ...
 void clearSampleChannel(int ch){std::lock_guard<std::mutex>l(m_);regions_.erase(std::remove_if(regions_.begin(),regions_.end(),[&](const Region&r){return r.channel==ch;}),regions_.end());voices_.erase(std::remove_if(voices_.begin(),voices_.end(),[&](const Voice&v){return v.channel==ch;}),voices_.end());sampleMode_=true;}
 ...
 void noteOn(int ch,int key,int vel){std::lock_guard<std::mutex>l(m_);if(!sampleMode_&&synth_){fluid_synth_noteon(synth_,ch,key,vel);return;}for(auto& r:regions_)if(ch==r.channel&&key>=r.loKey&&key<=r.hiKey&&vel>=r.loVel&&vel<=r.hiVel){Voice v;v.r=r;v.channel=ch;v.note=key;v.pos=r.offset;double semis=(key-r.keyCenter)+r.transpose+r.tune/100.0;v.step=std::pow(2.0,semis/12.0)*(double)r.sample->sampleRate/48000.0;float angle=(r.pan+1.f)*0.78539816339f;v.left=std::cos(angle)*r.gain*(vel/127.f);v.right=std::sin(angle)*r.gain*(vel/127.f);v.active=true;if(voices_.size()>=96)voices_.erase(voices_.begin());voices_.push_back(v);}}
 void noteOff(int ch,int key){std::lock_guard<std::mutex>l(m_);if(!sampleMode_&&synth_){fluid_synth_noteoff(synth_,ch,key);return;}for(auto&v:voices_)if(v.active&&v.channel==ch&&v.note==key)v.releasing=true;}
