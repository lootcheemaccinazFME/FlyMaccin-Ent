...
struct Region {
    std::shared_ptr<SampleData> sample; int channel=0; int loKey=0,hiKey=127,loVel=0,hiVel=127,keyCenter=60;
    float gain=1.f,pan=0.f,tune=0.f; int transpose=0,loopMode=0,loopStart=-1,loopEnd=-1,offset=0,end=-1; float release=.15f;
};
struct ChannelMix { float gain=1.f,pan=0.f; bool mute=false,solo=false; };
struct Voice { Region r; int channel=0,note=0; double pos=0,step=1; float left=1,right=1,env=1; bool releasing=false,active=false; };
...
public:
 ...
 int loadSf2(const std::string&p){std::lock_guard<std::mutex>l(m_);return synth_?fluid_synth_sfload(synth_,p.c_str(),1):-1;}
 void clearSamples(){std::lock_guard<std::mutex>l(m_);voices_.clear();regions_.clear();}
 void clearSampleChannel(int ch){std::lock_guard<std::mutex>l(m_);regions_.erase(std::remove_if(regions_.begin(),regions_.end(),[&](const Region&r){return r.channel==ch;}),regions_.end());voices_.erase(std::remove_if(voices_.begin(),voices_.end(),[&](const Voice&v){return v.channel==ch;}),voices_.end());}
 bool addRegion(const std::string&p,int ch,int lo,int hi,int lv,int hv,int kc,float db,float pan,float tune,int tr,int lm,int ls,int le,int off,int end,float rel){auto s=loadWav(p);if(!s)return false;Region r;r.sample=s;r.channel=ch;r.loKey=lo;r.hiKey=hi;r.loVel=lv;r.hiVel=hv;r.keyCenter=kc;r.gain=std::pow(10.f,db/20.f);r.pan=std::max(-100.f,std::min(100.f,pan))/100.f;r.tune=tune;r.transpose=tr;r.loopMode=lm;r.loopStart=ls;r.loopEnd=le;r.offset=std::max(0,off);r.end=end;r.release=std::max(.005f,rel);std::lock_guard<std::mutex>l(m_);regions_.push_back(std::move(r));return true;}
 void noteOn(int ch,int key,int vel){std::lock_guard<std::mutex>l(m_);bool sampled=false;for(auto& r:regions_)if(ch==r.channel&&key>=r.loKey&&key<=r.hiKey&&vel>=r.loVel&&vel<=r.hiVel){sampled=true;Voice v;v.r=r;v.channel=ch;v.note=key;v.pos=r.offset;double semis=(key-r.keyCenter)+r.transpose+r.tune/100.0;v.step=std::pow(2.0,semis/12.0)*(double)r.sample->sampleRate/48000.0;float angle=(r.pan+1.f)*0.78539816339f;v.left=std::cos(angle)*r.gain*(vel/127.f);v.right=std::sin(angle)*r.gain*(vel/127.f);v.active=true;if(voices_.size()>=96)voices_.erase(voices_.begin());voices_.push_back(v);}if(!sampled&&synth_)fluid_synth_noteon(synth_,ch,key,vel);}
 void noteOff(int ch,int key){std::lock_guard<std::mutex>l(m_);bool sampled=false;for(auto&v:voices_)if(v.active&&v.channel==ch&&v.note==key){v.releasing=true;sampled=true;}if(!sampled&&synth_)fluid_synth_noteoff(synth_,ch,key);}
 void programSelect(int ch,int sfid,int bank,int preset){std::lock_guard<std::mutex>l(m_);if(synth_)fluid_synth_program_select(synth_,ch,sfid,bank,preset);}
 ...
 void channelMix(int ch,float g,float p,bool mute,bool solo){std::lock_guard<std::mutex>l(m_);if(ch<0||ch>15)return;mix_[ch].gain=std::max(0.f,std::min(2.f,g));mix_[ch].pan=std::max(-1.f,std::min(1.f,p));mix_[ch].mute=mute;mix_[ch].solo=solo;}
 ...
 oboe::DataCallbackResult onAudioReady(oboe::AudioStream*,void*data,int32_t n) override {float*out=(float*)data;std::fill(out,out+n*2,0.f);std::lock_guard<std::mutex>l(m_);if(synth_)fluid_synth_write_float(synth_,n,out,0,2,out,1,2);for(auto&v:voices_){if(!v.active)continue;const Region&r=v.r;const auto&s=*r.sample;int last=r.end>=0?std::min(r.end,s.frames()-1):s.frames()-1;int ls=r.loopStart>=0?r.loopStart:0,le=r.loopEnd>ls?std::min(r.loopEnd,last):last;for(int i=0;i<n&&v.active;i++){int a=(int)v.pos;if(a>=last){if(r.loopMode&&le>ls){v.pos=ls; a=ls;}else{v.active=false;break;}}int b=std::min(a+1,last);float frac=float(v.pos-a);auto read=[&](int frame,int c){int cc=std::min(c,s.channels-1);return s.pcm[frame*s.channels+cc];};float L=read(a,0)+(read(b,0)-read(a,0))*frac;float R=s.channels>1?(read(a,1)+(read(b,1)-read(a,1))*frac):L;if(v.releasing){float dec=1.f/(48000.f*std::max(.005f,r.release));v.env=std::max(0.f,v.env-dec);if(v.env<=0){v.active=false;break;}}bool anySolo=false;for(int c=0;c<16;c++)anySolo=anySolo||mix_[c].solo;const auto&mx=mix_[std::max(0,std::min(15,v.channel))];if(!mx.mute&&(!anySolo||mx.solo)){float a=(mx.pan+1.f)*0.78539816339f;out[i*2]+=L*v.left*v.env*master_*mx.gain*std::cos(a)*1.41421356f;out[i*2+1]+=R*v.right*v.env*master_*mx.gain*std::sin(a)*1.41421356f;}v.pos+=v.step;if(r.loopMode&&v.pos>=le)v.pos=ls+(v.pos-le);}}
...
private:std::mutex m_;fluid_settings_t*settings_=nullptr;fluid_synth_t*synth_=nullptr;std::shared_ptr<oboe::AudioStream>stream_;std::vector<Region>regions_;std::vector<Voice>voices_;ChannelMix mix_[16];float master_=.85f;
};
...
extern "C" JNIEXPORT void JNICALL Java_com_flymaccin_demonicdaw_NativeAudioEngine_nativeClearSampleChannel(JNIEnv*,jclass,jint c){E.clearSampleChannel(c);}
extern "C" JNIEXPORT jboolean JNICALL Java_com_flymaccin_demonicdaw_NativeAudioEngine_nativeAddWavRegion(JNIEnv*e,jclass,jstring s,jint ch,jint lo,jint hi,jint lv,jint hv,jint kc,jfloat db,jfloat pan,jfloat tune,jint tr,jint lm,jint ls,jint le,jint off,jint end,jfloat rel){const char*p=e->GetStringUTFChars(s,0);bool ok=E.addRegion(p,ch,lo,hi,lv,hv,kc,db,pan,tune,tr,lm,ls,le,off,end,rel);e->ReleaseStringUTFChars(s,p);return ok;}
