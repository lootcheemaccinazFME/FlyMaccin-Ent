...
layer = '<div id="fme-global-bg-layer" aria-hidden="true"><img src="file:///android_asset/fme_qr_hub_bg.webp" alt=""></div>\n'
...
if 'id="fme-global-bg-layer"' not in html:
    html = re.sub(r'<body([^>]*)>', lambda m: m.group(0) + '\n' + layer, html, count=1, flags=re.I)
...
if 'id="fme-global-bg-layer"' not in book_html:
    book_html = re.sub(r'<body([^>]*)>', lambda m: m.group(0) + '\n' + layer, book_html, count=1, flags=re.I)
...
print('Applied v2.1.8 real fixed FME background image layer to main studio and Book Writer.')
